...
You may want to call this fn while in a log buffer, with point on a commit hash."
  ...
  (let ((commit (or commit
    ...
    (when (not (null commit))
      ...
      (fugitive-shell-command (concat "git describe --tags --candidates=10 " commit)
                              nil
                              t ; give a chance to modify command in minibuffer
                              ))))
...
Does not include diffs for merge commits as they tend to have large output, freezing/crashing Emacs."
  ...
  (let ((commit (or commit
    ...
    (when (not (null commit))
      ...
      (fugitive-shell-command (concat fugitive-core-git-show-cmd
...
(defun fugitive-show-merge-commit (&optional commit)
  "Like `fugitive-show' but output options tailored for merge commits."
  (interactive)
  (let ((commit (or commit
                    (fugitive-hash-or-next-line))))
    (when (not (null commit))
