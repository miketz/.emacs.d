...
You may want to call this fn while in a log buffer, with point on a commit hash."
  ...
  (let ((commit (or commit
    ...
    (when (not (null commit))
      ...
      (fugitive-shell-command (concat "git show --stat --first-parent -p --histogram --color-moved=zebra --pretty=format:\"%C(auto)%H%nParents: %p%nRefs: %d%n[38;5;74mAuthor:%C(auto) %an <%ae>%n        %C(auto)%ad%n[38;5;74mCommit:%C(auto) %cn <%ce>%n        %cd %n%n%w(0,3,3)%B\" --date=iso " commit)))))
